...
> **Status: Phase 5 — Conversational agent (in progress).** Phases 1–4 are complete: architecture ([docs/](docs/)), auth/tenancy/menu on **RLS** ([docs/PHASE-2.md](docs/PHASE-2.md)), the ingestion pipeline ([docs/PHASE-3.md](docs/PHASE-3.md)), and knowledge/RAG on pgvector ([docs/PHASE-4.md](docs/PHASE-4.md)). Phase 5 adds the agent brain — `ordy-agent` (typed conversation state, supervisor routing, a deterministic dev brain + an LLM brain behind a port, grounding enforced in code), conversation persistence, a **text sandbox** API + chat UI that answers menu/hours questions grounded in approved knowledge, and the voice worker scaffolded behind STT/TTS/transport ports ([docs/PHASE-5.md](docs/PHASE-5.md)). Read-only; tool calling is Phase 6.
